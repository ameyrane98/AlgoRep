...
const syncStats = async () => {
  let { algorep_hook, algorep_token, sync_stats, stats } = await api.storage.local.get([
    'algorep_token',
    'algorep_hook',
    'sync_stats',
    'stats',
  ...
  let options = {
    method: 'GET',
    headers: {
      Authorization: `token ${algorep_token}`,
...
const createRepo = async (token, name) => {
  ...
  api.storage.local.set({ mode_type: 'commit', algorep_hook: res.full_name });
...
const getLinkErrorString = (statusCode, name) => {
  ...
  const errorStrings = {
    301: `Error linking <a target="blank" href="${`https://github.com/${name}`}">${name}</a> to AlgoRep. <br> This repository has been moved permenantly. Try creating a new one.`,
    403: `Error linking <a target="blank" href="${`https://github.com/${name}`}">${name}</a> to AlgoRep. <br> Forbidden action. Please make sure you have the right access to this repository.`,
    404: `Error linking <a target="blank" href="${`https://github.com/${name}`}">${name}</a> to AlgoRep. <br> Resource not found. Make sure you enter the right repository name.`,
  };
  return errorStrings[statusCode];
};
  ...
  xhr.addEventListener('readystatechange', function () {
    ...
    if (xhr.status !== 200) {
      ...
      api.storage.local.set({ mode_type: 'hook', algorep_hook: null }, () => {
        console.log(`Error linking ${name} to AlgoRep`);
    ...
    api.storage.local.set(
      ...
      () => {
        $('#error').hide();
        $('#success').html(
          `Successfully linked <a target="blank" href="${res.html_url}">${name}</a> to AlgoRep. Start <a href="http://leetcode.com">LeetCoding</a> now!`
        );
        $('#success').show();
        $('#unlink').show();
...
const unlinkRepo = () => {
  ...
  api.storage.local.set(
    { mode_type: 'hook', algorep_hook: null, sync_stats: true, stats: null },
    () => {
    ...
    */
    api.storage.local.get('algorep_token', data => {
      ...
      if (token === null || token === undefined) {
        /* Not authorized yet. */
        $('#error').text(
          'Authorization error - Grant AlgoRep access to your GitHub account to continue (launch extension to proceed)'
        );
        $('#error').show();
        $('#success').hide();
      } else if (option() === 'new') {
        ...
        api.storage.local.get('algorep_username', data2 => {
          const username = data2.algorep_username;
          if (!username) {
            /* Improper authorization. */
            $('#error').text(
              'Improper Authorization error - Grant AlgoRep access to your GitHub account to continue (launch extension to proceed)'
            );
            $('#error').show();
            $('#success').hide();
          } else {
...
api.storage.local.get('mode_type', data => {
  ...
  if (mode && mode === 'commit') {
    ...
    api.storage.local.get('algorep_token', data2 => {
      const token = data2.algorep_token;
      if (token === null || token === undefined) {
        /* Not authorized yet. */
        $('#error').text(
          'Authorization error - Grant AlgoRep access to your GitHub account to continue (click AlgoRep extension on the top right to proceed)'
        );
        $('#error').show();
        $('#success').hide();
        /* Hide accordingly */
        ...
        /* Get access to repo */
        api.storage.local.get('algorep_hook', repoName => {
          const hook = repoName.algorep_hook;
          if (!hook) {
            /* Not authorized yet. */
            $('#error').text(
              'Improper Authorization error - Grant AlgoRep access to your GitHub account to continue (click AlgoRep extension on the top right to proceed)'
            );
            $('#error').show();
            $('#success').hide();
            /* Hide accordingly */
